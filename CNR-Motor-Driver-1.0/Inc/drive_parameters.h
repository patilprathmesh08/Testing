...
#define REGULATION_EXECUTION_RATE           2 /*!< FOC at 25kHz with 50kHz PWM. The ~16.8us ISR fits the 40us window; rate=1 (50kHz) overran the 20us window and raised MC_DURATION. */
#define ISR_FREQUENCY_HZ                    (PWM_FREQUENCY/REGULATION_EXECUTION_RATE) /*!< @brief FOC execution rate in Hz */

/* Gains values for torque and flux control loops */
#define PID_TORQUE_KP_DEFAULT               1400
...
#define PID_SPEED_KP_DEFAULT                200/(SPEED_UNIT/10) /* Workbench compute the gain for 01Hz unit*/
...
#define PID_POSITION_KP_GAIN                30   /* Effective gain 30/1024 = 0.0293, matched to the reference position P gain (~0.030). */
...
#define FINAL_I_ALIGNMENT_A                 0.05 /*!< Low alignment current. EAC alignment is time-based (fixed duration), so it still completes the start sequence, but at this low current it does not noticeably swing the rotor beforehand, giving a consistent, repeatable rotor start position. */
