...
#include "mc_type.h"
/* Count of rejected encoder glitches (see the glitch reject in ENC_CalcAngle). */
volatile uint32_t g_enc_glitch_count = 0;
...
__weak int16_t ENC_CalcAngle(ENCODER_Handle_t *pHandle)
{
  ...
  else
  {
    ...
    /* PWM-noise glitch reject. Under drive, PWM switching can couple onto the
     * encoder A/B lines and inject false counts, corrupting the angle for one
     * HF cycle so the position loop sees a phantom jump and kicks the rotor.
     * At the HF task rate a real mechanical step is tiny; a jump larger than
     * ~1/8 rev (8192 s16) in one cycle is physically impossible for this motor
     * and is therefore treated as noise: hold the previous mechanical angle for
     * this cycle. This guards both electrical angle and position, which both
     * derive from mecAngle here. */
    {
      int16_t dbg = (int16_t)(mecAngle - hMecAnglePrev);
      int16_t adbg = (dbg < 0) ? (int16_t)(-dbg) : dbg;
      if (adbg > 8192)
      {
        mecAngle = hMecAnglePrev;   /* impossible jump -> hold last good */
        g_enc_glitch_count++;       /* track how often glitches are rejected */
      }
    }
