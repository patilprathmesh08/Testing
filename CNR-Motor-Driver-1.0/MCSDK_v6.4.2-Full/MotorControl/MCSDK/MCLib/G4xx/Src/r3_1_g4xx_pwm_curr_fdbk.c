...
__STATIC_INLINE uint16_t R3_1_WriteTIMRegisters(PWMC_Handle_t *pHdl, uint16_t SamplingPoint)
{
  ...
  /* CNR board: PhA duty -> CH3 (PA10->INA), PhC duty -> CH1 (PA8->INC).
   * Current-sense channels swapped correspondingly in mc_parameters.c.
   * This double-swap config gave the best result (Id~0, RPM=1500 achieved). */
  LL_TIM_OC_SetCompareCH1(TIMx, (uint32_t) pHandle->_Super.CntPhC);
  LL_TIM_OC_SetCompareCH2(TIMx, (uint32_t) pHandle->_Super.CntPhB);
  LL_TIM_OC_SetCompareCH3(TIMx, (uint32_t) pHandle->_Super.CntPhA);
  LL_TIM_OC_SetCompareCH4(TIMx, (uint32_t) SamplingPoint);

  /* Limit for update event */

//  if ( LL_TIM_CC_IsEnabledChannel(TIMx, LL_TIM_CHANNEL_CH4) == 1u )
  /* MC_DURATION when TRGO still=OC4REF (TIM1_UP preempted this ISR between GetPhaseCurrents and here).
     MC_NO_ERROR when TRGO=RESET (GetPhaseCurrents already disarmed the trigger - normal case). */
