...
/**
  * @brief  This function handles M1 Encoder Index IRQ on PIN PB6.
  */
...
void EXTI9_5_IRQHandler(void)
{
  ...
  if (0U != LL_EXTI_ReadFlag_0_31(LL_EXTI_LINE_6))
  {
    ...
    /* Capture the raw TIM4 count at the first encoder Z (index) crossing.
     * The index is a fixed mechanical mark, so this count is a repeatable
     * absolute position reference for later use by position control. The
     * counter is not reset here, and only the first index is captured. */
    extern volatile int16_t g_index_enc;
    extern volatile uint8_t g_index_seen;

    if (0U == g_index_seen)
    {
      g_index_enc  = (int16_t)LL_TIM_GetCounter(TIM4);
      g_index_seen = 1U;
    }
  }
  else
