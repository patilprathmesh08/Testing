...
/* ISR cycle-time instrumentation to size the 50kHz MC_DURATION budget.
 * DWT->CYCCNT is a free-running CPU cycle counter. We bracket the HF FOC
 * chain and record the maximum cycle count seen, printed from the main loop.
 * At 170MHz a 20us period (50kHz) is ~3400 cycles of budget. */
volatile uint32_t g_isr_cyc_max = 0;
volatile uint32_t g_isr_cyc_last = 0;
static inline void DWT_CycInit(void)
{
...
__weak uint8_t FOC_HighFrequencyTask(uint8_t bMotorNbr)
{
  ...
  static uint8_t dwt_init_done = 0u;
  if (dwt_init_done == 0u) { DWT_CycInit(); dwt_init_done = 1u; }
  uint32_t cyc_start = DWT->CYCCNT;

  /* RCM regular-conversion management is intentionally NOT run in the HF task.
   * It would schedule Vbus/temperature ADC conversions each cycle, but this
   * build uses a virtual bus-voltage sensor and virtual temperature - nothing
   * in the control loop consumes a real regular conversion. Skipping it frees
   * ~100-200 cycles per ISR so the FOC chain fits inside the 20us period. No
   * protection path reads these conversions, so it is safe to omit. */
  /* RCM_ReadOngoingConv();  (removed - virtual bus) */
  /* RCM_ExecNextConv();     (removed - virtual bus) */
  ...
  {
    uint32_t cyc = DWT->CYCCNT - cyc_start;
    g_isr_cyc_last = cyc;
    if (cyc > g_isr_cyc_max) { g_isr_cyc_max = cyc; }
  }
...
inline uint16_t FOC_CurrControllerM1(void)
{
  ...
  /* Capture the raw (pre-offset) electrical angle for logging by reusing the
   * value just fetched, rather than calling SPD_GetElAngle() again. At 50kHz
   * the FOC ISR must complete within 20us or TIM1_UP preempts it and R3_1
   * returns MC_DURATION, so the duplicate computation is avoided in the hot
   * path. */
  extern volatile int16_t g_el_raw_dbg;
  g_el_raw_dbg = hElAngle;   /* raw pre-offset angle */
  /* The PWM-noise glitch reject lives at the source in ENC_CalcAngle (which
   * guards both electrical angle and position from one point), so no guard is
   * needed here. */
  /* Apply the commutation offset ONLY in RUN state. During ALIGNMENT the
   * MCSDK forces a known angle to reference the rotor; adding the offset there
   * corrupts the alignment, so it is gated to RUN. */
  extern volatile int16_t g_el_offset_s16;
  if (Mci[M1].State == RUN)
  {
    hElAngle = (int16_t)(hElAngle + g_el_offset_s16);
  }
   ...
   * shift; larger N = heavier filtering. Tunable. */
  static int32_t iq_filt = 0, id_filt = 0;
  iq_filt += ((int32_t)Iqd.q - iq_filt) >> 5;
  id_filt += ((int32_t)Iqd.d - id_filt) >> 5;
  Iqd.q = (int16_t)iq_filt;
  Iqd.d = (int16_t)id_filt;
